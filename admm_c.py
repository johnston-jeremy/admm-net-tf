...
class ADMMNet(tf.keras.Model):
  ...
  def call(self, inputs):
    z_0 = (np.zeros((self.n1,1),dtype=np.float32), np.zeros((self.n1,1),dtype=np.float32))
    u_0 = (np.zeros((self.n1,1),dtype=np.float32), np.zeros((self.n1,1),dtype=np.float32))
...
class x_update(layers.Layer):
  ...
  def __init__(self, params_init, p, *args, **kwargs):
    ...
    if 'blank' in args:
      M1_re_init = np.zeros((n,m))
      M1_im_init = np.zeros((n,m))    
      M2_re_init = np.zeros((n,n))
      M2_im_init = np.zeros((n,n))
      M3_init = np.zeros((n,))
    else:
      if m < n:
        AULA = self.AULA(p)
        M1_init = np.matmul(np.eye(n)/self.rho0 - (1/self.rho0**2)*AULA, p.A.T.conj())
        M1_re_init = np.concatenate(np.real(M1_init), np.imag(M1_init))
        M1_imag_init = np.concatenate(-np.imag(M1_init), np.real(M1_init))
        M2_init = (1/self.rho0)*np.eye(n) - (1/self.rho0**2)*AULA
        M2_re_init = np.concatenate(np.real(M2_init), np.imag(M2_init))
        M2_imag_init = np.concatenate(-np.imag(M2_init), np.real(M2_init))
      else:
        # UL = self.invUinvL(p)
    ...
    self.M1_re = tf.Variable(initial_value=M1_re_init.astype(np.float32),
                         trainable=not tied, name='M1_re')
    self.M1_imag = tf.Variable(initial_value=M1_imag_init.astype(np.float32),
                         trainable=not tied, name='M1_imag')
    self.M2_re = tf.Variable(initial_value=M2_re_init.astype(np.float32),
                         trainable=not tied, name='M2_re')
    self.M2_imag = tf.Variable(initial_value=M2_imag_init.astype(np.float32),
                         trainable=not tied, name='M2_imag')
  ...
  def call(self, y, z, u):
    y_re = np.real(y)
    y_im = np.imag(y)
    x1_re = tf.matmul(self.M1_re, tf.transpose(y_re))
    x1_imag = tf.matmul(self.M1_imag, tf.transpose(y_im))
    x2_re = tf.matmul(self.M2_re, self.rho*(z[0]-u[0]))
    x2_imag = tf.matmul(self.M2_imag, self.rho*(z[1]-u[1]))
    x_re = x1_re + x2_re
    x_imag = x1_imag + x2_imag
    ...
    return self.alph*x_re + (1-self.alph)*z[0], self.alph*x_imag + (1-self.alph)*z[1]
...
class z_update(layers.Layer):
  ...
  def call(self, x_1, u):
    z_re = tfp.math.soft_threshold(x_1[0]+u[0], self.lamb/self.rho)
    z_im = tfp.math.soft_threshold(x_1[1]+u[1], self.lamb/self.rho)
    return z_re, z_im 
...
class u_update(layers.Layer):
  ...
  def call(self, x_1, z_1, u):
    u_re = u[0] + x_1[0] - z_1[0]
    u_re = u[1] + x_1[1] - z_1[1]
    return u_re, u_im
